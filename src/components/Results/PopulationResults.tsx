...
const PopulationResults: React.FC<Props> = ({ results, id }) => {
    ...
    const rows = [
        `${fhirpath.evaluate(patientResource, 'Patient.name.given')} ${fhirpath.evaluate(patientResource, 'Patient.name.family')}`, 
    ...
    return (
        <Table className={classes.table} size="small">
            <TableHead>
                <TableRow>
                    {columns.map((column) => (
                        <TableCell className={classes.tableCell} align="left">{column}</TableCell>
                    ))}
                </TableRow>
            </TableHead>
            <TableBody> 
